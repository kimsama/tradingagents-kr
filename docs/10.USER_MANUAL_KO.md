...
이 섹션은 컨테이너가 어떤 명령을 노출하고, 기본 `CMD`를 어떻게 덮어쓸 수 있는지 한 곳에 정리합니다.

### 18.1 진입점 동작

이미지의 `ENTRYPOINT`는 `/usr/local/bin/docker-entrypoint.sh`이고, 기본 `CMD`는 `tradingagents`입니다. 컨테이너가 시작되면 `docker-entrypoint.sh`가 다음을 수행합니다.

1. `${HOME}/.tradingagents/{cache,logs,memory}` 디렉터리를 생성하고, 컨테이너가 root로 실행 중이면 소유권을 `appuser`로 변경합니다.
2. 다음 환경변수가 있으면 해당 경로를 사용하고, 없으면 위 기본값을 사용합니다.
   - `TRADINGAGENTS_CACHE_DIR`
   - `TRADINGAGENTS_RESULTS_DIR`
   - `TRADINGAGENTS_MEMORY_LOG_PATH`
3. `setpriv`로 `appuser`로 권한을 낮춘 뒤 명령을 `exec`합니다.
4. 사용자가 `-`로 시작하는 인수(예: `--help`)나 컨테이너에 존재하지 않는 명령을 전달하면 자동으로 앞에 `tradingagents`를 붙입니다.

### 18.2 기본 명령: `tradingagents`

`tradingagents`는 Typer 기반 CLI(`cli.main:app`)로, 단일 서브커맨드 `analyze`와 Typer가 자동으로 제공하는 도움말/완성 옵션을 노출합니다.

| 명령 | 역할 |
| --- | --- |
| `tradingagents` | 도움말을 표시합니다(서브커맨드 없이 호출 시). |
| `tradingagents analyze` | 대화형 8단계 위저드를 실행하고 분석을 시작합니다. |
| `tradingagents --help` | 전체 도움말을 표시합니다. |
| `tradingagents --install-completion` | 셸 자동완성을 설치합니다(컨테이너 안에서는 거의 사용하지 않음). |
| `tradingagents --show-completion` | 셸 자동완성 스크립트를 출력합니다. |

### 18.3 `analyze` 서브커맨드 옵션

`analyze`가 노출하는 플래그는 두 개뿐입니다. 나머지(티커, 날짜, 언어, 모델, Provider 등)는 모두 §5 위저드에서 입력받습니다.

| 플래그 | 기본값 | 설명 |
| --- | --- | --- |
| `--checkpoint` | off | LangGraph 체크포인트/재개를 활성화합니다. 각 노드 완료 후 상태가 저장되어, 크래시 발생 시 동일한 ticker+date로 다시 실행하면 마지막 성공 단계부터 재개됩니다. |
| `--clear-checkpoints` | off | 실행 전에 `${cache_dir}/checkpoints/` 아래의 모든 체크포인트 DB를 삭제합니다. |
| `--help` | — | `analyze` 서브커맨드 도움말을 표시합니다. |

체크포인트의 위치와 동작 세부사항은 §10을 참조하세요.

### 18.4 기본 `CMD` 덮어쓰기

진입점은 사용자가 전달한 인수를 그대로 전달합니다. 따라서 다음과 같이 사용할 수 있습니다.

도움말 확인:

```bash
docker run --rm tradingagents-kr-tradingagents tradingagents --help
docker run --rm tradingagents-kr-tradingagents tradingagents analyze --help
```
...
```bash
docker run -it --rm \
  -e OPENAI_AUTH_MODE=api_key -e OPENAI_API_KEY \
  -v tradingagents_data:/home/appuser/.tradingagents \
  tradingagents-kr-tradingagents \
  tradingagents analyze --checkpoint
```

진입점이 `-`로 시작하는 인수를 알아서 처리하므로 다음도 동일하게 동작합니다.

```bash
docker run --rm tradingagents-kr-tradingagents --help
```

컨테이너 안 셸로 진입(디버깅·로그 확인용):

```bash
docker run -it --rm tradingagents-kr-tradingagents sh
docker exec -it tradingagents-test sh
```
...
```bash
docker run -it --rm tradingagents-kr-tradingagents python
```

### 18.5 docker-compose 서비스

`docker-compose.yml`은 두 서비스를 정의합니다.

| 서비스 | 활성화 조건 | 용도 |
| --- | --- | --- |
| `tradingagents` | 기본 | 일반 분석 실행. `.env`에서 Provider 키와 `*_AUTH_MODE`를 읽습니다. |
| `tradingagents-ollama` | `--profile ollama` | 로컬 Ollama 컨테이너(`ollama` 서비스)와 함께 실행. `LLM_PROVIDER=ollama`가 자동 설정되며, Ollama 컨테이너에 의존합니다. |
...
두 서비스 모두 `tradingagents_data` named volume을 사용해 보고서·캐시·메모리 로그를 보존합니다.
